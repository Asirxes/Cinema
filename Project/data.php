...
?>
    ...
    <div class="w3-main" style="margin-left:250px;margin-bottom: 70px">
        <div class="w3-row w3-grayscale mt-5 text-center">
            <h3><b>Zmieniasz datę lub ilość dla:</b><br/>
                <?php
                $exploded = explode("-",$_GET['date']);
                $date = $exploded[2]."-".$exploded[1]."-".$exploded[0];
                echo $_GET['film']." ".$date;
                echo "<br/>";
                echo $_GET['ilosc']." BILET/Y/ÓW";
                ?></h3>
        </div>
        <div class="w3-row w3-grayscale">
            <?php
            echo "<form style='margin-left: 10px;margin-right: 10px' action='akcje/changeDate.php' method='post' class='mb-5'>";
            ?>
            <div class="form-group mt-5">
                ...
                echo "<input type='text' class='form-control' id='oldDate' aria-describedby='oldDate' name='oldDate' hidden value='$data'>";
                echo "<input type='text' class='form-control' id='oldIlosc' aria-describedby='oldIlosc' name='oldIlosc' hidden value='$ilosc'>";
                ?>
...
if (isset($_GET["error"]) && $_GET["error"] == "true") {
    echo '<script>window.onload=function(){alert("Już masz rezerwację na podany film, ilosc oraz datę, usuń pierwszą rezerwację aby dokonać następnej!")}</script>';
}
