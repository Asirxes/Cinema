...
<div class="w3-main" style="margin-left:250px;position: relative;top: 50px;">
    ...
    <form style="margin-left: 10px;margin-right: 10px" action="akcje/zarejestruj.php" method="post">
...
if (isset($_GET["error"]) && $_GET["error"] == "true") {
    echo '<script>window.onload=function(){alert("Login już zajęty!")}</script>';
}

if (isset($_GET["error"]) && $_GET["error"] == "login") {
    echo '<script>window.onload=function(){alert("Aby dokonać rejestracji musisz założyć konto!")}</script>';
}

if (isset($_GET["error"]) && $_GET["error"] == "rezerwacje") {
    echo '<script>window.onload=function(){alert("Dostęp tylko dla zalogowanych użytkowników!")}</script>';
}
